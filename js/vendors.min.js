...
if (window.innerWidth < 800) {
	let title = document.querySelectorAll('.item-program__title');
	let block = document.querySelectorAll('.item-program__block');
	let h = [];
	for (let index = 0; index < title.length; index++) {
		const element = title[index];
		h.push(block[index].clientHeight)
		block[index].style.height = '0px'
		block[index].style.padding = '0px'
		element.addEventListener('click', function () {
			element.classList.toggle('_active')
			if (element.classList.contains('_active')) {

				block[index].style.height = `${h[index]}px`
				if (window.innerWidth < 500) {
					block[index].style.padding = '25px 20px'
				} else {
					block[index].style.padding = '35px'
				}
...
let quest = document.querySelectorAll('.faq__quest');
let resp = document.querySelectorAll('.faq__resp');
